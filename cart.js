...
(function () {
  ...
  /* ── styles ── */
  var css = [
    '.cart-backdrop { position: fixed; inset: 0; background: rgba(62,42,26,0.45); opacity: 0; pointer-events: none; transition: opacity .35s ease; z-index: 9000; }',
    '.cart-backdrop.open { opacity: 1; pointer-events: auto; }',
    '.cart-drawer { position: fixed; top: 0; right: 0; bottom: 0; width: min(420px, 100vw); background: var(--cream); z-index: 9001; display: flex; flex-direction: column; transform: translateX(100%); transition: transform .45s cubic-bezier(.22,1,.36,1); box-shadow: -8px 0 30px rgba(62,42,26,0.2); }',
    ...
    '.cart-stitch { background-image: url("data:image/svg+xml,%3Csvg width=\'12\' height=\'2\' xmlns=\'http://www.w3.org/2000/svg\'%3E%3Cline x1=\'0\' y1=\'1\' x2=\'6\' y2=\'1\' stroke=\'%233E2A1A\' stroke-width=\'1.5\' stroke-linecap=\'round\'/%3E%3C/svg%3E"); background-repeat: repeat-x; height: 2px; opacity: .3; margin: 0 28px; }',
    '.cart-ship { padding: 18px 28px 6px; }',
    '.cart-ship-msg { font-size: 12px; color: var(--walnut); margin-bottom: 10px; letter-spacing: .04em; }',
    '.cart-ship-msg strong { color: var(--terra); font-weight: 500; }',
    '.cart-ship-track { height: 4px; background: var(--linen); overflow: hidden; }',
    '.cart-ship-fill { height: 100%; background: var(--terra); width: 0; transition: width .5s ease; }',
