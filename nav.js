...
(function () {
  ...
  var css = [
    ...
    '}',
    '.mnav-backdrop { position: fixed; inset: 0; background: rgba(106,70,48,0.45); opacity: 0; pointer-events: none; transition: opacity .35s ease; z-index: 9400; }',
    '.mnav-backdrop.open { opacity: 1; pointer-events: auto; }',
    '.mnav { position: fixed; top: 0; right: 0; bottom: 0; width: min(320px, 85vw); background: var(--cream); z-index: 9401; padding: 76px 36px 36px; transform: translateX(100%); transition: transform .45s cubic-bezier(.22,1,.36,1); box-shadow: -8px 0 30px rgba(106,70,48,0.2); display: flex; flex-direction: column; }',
